...
# Для aiogram_socks (SOCKS5) — установи пакет: pip install aiohttp_socks
# ===================== ПРОКСИ =====================
PROXY_HOST = "178.171.43.116"
PROXY_PORT = 9330
PROXY_USER = "JktV0d"
PROXY_PASS = "vXFVn4"

# Единый прокси для всего (SOCKS5 с авторизацией)
PROXY_URL = f"socks5://{PROXY_USER}:{PROXY_PASS}@{PROXY_HOST}:{PROXY_PORT}"
PROXY_TUPLE = (socks.SOCKS5, PROXY_HOST, PROXY_PORT, True, PROXY_USER, PROXY_PASS)  # для Telethon
# ===================== НАСТРОЙКИ =====================
...
OPENROUTER_API_KEY = "САМИ БАЛЯ ДОСТАНЬТЕ!!!!"
...
telethon_client = TelegramClient(
    ...
    API_ID, 
    API_HASH, 
    proxy=PROXY_TUPLE   # или можно использовать dict: proxy={"proxy_type": "socks5", "addr": PROXY_HOST, "port": PROXY_PORT, "username": PROXY_USER, "password": PROXY_PASS}
)
# Глобальные клиенты
session = AiohttpSession(proxy=PROXY_URL)  # прокси для aiogram (user_bot и mod_bot)

user_bot = Bot(token=USER_TOKEN, session=session)
mod_bot = Bot(token=MOD_TOKEN, session=session)
# ===================== ТЕКСТЫ ЖАЛОБ (3 варианта на каждую причину) =====================
...
def init_db():
    try:
        ...
        cur.execute("INSERT OR IGNORE INTO moderators (user_id, username, added_by) VALUES (?,?,?)",
                    (MOD_ID, "Главный модератор", MOD_ID))
        conn.commit()
        conn.close()
...
def set_config(key: str, value: str | None):
    try:
        ...
        print(f"Ошибка сохранения конфига '{key}': {e}")
...
async def mod_timeout_processor():
    print("⏰ Фоновая проверка таймаутов модераторов запущена (5 минут)")
    while True:
        ...
        try:
            ...
            for report_id, old_mod_id in rows:
                print(f"⏰ Таймаут репорта #{report_id} (мод {old_mod_id}) — эскалирован главному модератору")
...
def insert_report(user_id: int, username: str, description: str, reason: str, link: str) -> int:
    try:
        ...
        print(f"Ошибка вставки репорта: {e}")
        return -1
...
def update_report_ai_info(report_id: int, reason_type: str, confidence: float, explanation: str):
    try:
        ...
        print(f"Ошибка обновления AI info: {e}")
...
def build_initial_mod_kb(report_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="❌ отклонить", callback_data=f"mod_action:{report_id}:reject")],
        [InlineKeyboardButton(text="✅ принять на рассмотрение", callback_data=f"mod_action:{report_id}:accept_review")],
        [InlineKeyboardButton(text="⏭ отложить", callback_data=f"mod_action:{report_id}:next")]
    ])

def build_action_mod_kb(report_id: int, report_type: str, mod_id: int) -> InlineKeyboardMarkup:
    kb = [
        [InlineKeyboardButton(text="🚨 принять меры", callback_data=f"mod_action:{report_id}:measures")],
        [InlineKeyboardButton(text="⛔ забанить", callback_data=f"mod_action:{report_id}:ban")]
    ]
...
async def send_broadcast_to_users(text: str):
    ...
    for uid in users:
        try:
            await user_bot.send_message(uid, f"📢 <b>Рассылка от администрации:</b>\n\n{text}", parse_mode="HTML")
            success += 1
            await asyncio.sleep(0.08)
        except:
            pass
    await mod_bot.send_message(MOD_ID, f"✅ Рассылка пользователям завершена!\nДоставлено: {success}/{len(users)}")
...
async def send_broadcast_to_mods(text: str):
    ...
    for mid in mods:
        try:
            await mod_bot.send_message(mid, f"📢 <b>Рассылка модераторам:</b>\n\n{text}", parse_mode="HTML")
            success += 1
        except:
            pass
    await mod_bot.send_message(MOD_ID, f"✅ Рассылка модераторам завершена!\nДоставлено: {success}/{len(mods)}")
...
async def notify_all_mods_new_report(report_id: int):
    try:
        ...
        if report.get("ai_reason_type"):
            ai_info = f"""🤖 <b>АНАЛИЗ ИИ:</b>
📌 Причина: <b>{report['ai_reason_type'].upper()}</b>
📊 Уверенность: {report.get('ai_confidence', 0):.2f}
💡 Пояснение: {report.get('ai_explanation', '—')}
🔧 Метод жалобы: <code>InputReportReason{report['ai_reason_type'].capitalize()}()</code>
"""

        text = f"""🆕 <b>НОВЫЙ РЕПОРТ #{report_id} В ОБЩЕЙ ОЧЕРЕДИ</b>
👤 <b>От пользователя:</b> {username_display} (<code>{report['user_id']}</code>)
📌 <b>Тип:</b> {report_type.upper()}
📝 <b>Описание:</b> {report['description'][:100]}...
🔗 <b>Ссылка:</b> {report['link']}
❓ <b>Причина:</b> {report['reason']}
{ai_info}
<i>Используйте /next чтобы взять репорт на рассмотрение 👇</i>"""
        ...
        for mid in get_moderators():
            try:
                ...
                pass
        print(f"✅ Новый репорт #{report_id} уведомлён всем модераторам (с анализом ИИ)")
    except Exception as e:
        print(f"Ошибка уведомления модераторов: {e}")
...
async def assign_and_notify_mod(report_id: int, forced_mod: int | None = None):
    try:
        ...
        ai_info = ""
        if report.get("ai_reason_type"):
            ai_info = f"""🤖 <b>АНАЛИЗ ИИ:</b> {report['ai_reason_type'].upper()} ({report.get('ai_confidence',0):.2f})
🔧 Метод: InputReportReason{report['ai_reason_type'].capitalize()}()
"""

        text = f"""<b>📋 ЗАЯВКА #{report_id} НАЗНАЧЕНА ВАМ</b>
👤 <b>От пользователя:</b> {username_display} (<code>{report['user_id']}</code>)
📌 <b>Тип:</b> {report_type.upper()}
...
<i>Выберите действие ниже 👇</i>"""
        ...
        print(f"✅ Репорт #{report_id} назначен модератору {mod_id}")
    except Exception as e:
        print(f"Ошибка assign_and_notify_mod {report_id}: {e}")
...
async def get_or_create_ai_status_message() -> int:
    ...
    msg = await mod_bot.send_message(
        MOD_ID,
        "🤖 <b>ИИ-модерация очереди ботов</b>\n\nСтатус: <i>ожидание...</i>",
        parse_mode="HTML"
    )
...
async def analyze_bot_with_ai(description: str, reason: str, link: str, bot_messages: list[str]) -> dict:
    prompt = f"""Ты модератор TG. Выбери только причину (не принимай решение).
Описание: {description}
Причина: {reason}
Ссылка: {link}
Сообщения: {chr(10).join([f"[{i+1}] {msg}" for i, msg in enumerate(bot_messages)]) if bot_messages else "Бот НЕ ответил"}
Ответь JSON:
{{"reason_type": "personal_details"/"spam"/"other", "confidence": 0.0-1.0, "explanation": "max 70 chars"}}"""
    ...
    for api_key in keys:
        try:
            ...
            continue
    return {"reason_type": "other", "confidence": 0.0, "explanation": "Ошибка анализа"}
...
async def send_4_complaints(bot_username: str, reason_type: str = "personal_details"):
    ...
    try:
        ...
        print(f"✅ Отправлено 4 жалобы на @{bot_username} методом {reason_type}")
    except Exception as e:
        print(f"Ошибка отправки жалоб: {e}")
...
async def ai_moderate_bot(report_id: int):
    ...
    await update_ai_status(f"""🤖 <b>ИИ анализирует бота</b>
🔗 @{bot_username}
📋 Репорт: #{report_id}
⏳ Собираем сообщения...""")
    ...
    try:
        ...
        if not bot_messages:
            ...
            await user_bot.send_message(user_id, "Меры не были приняты, модератор не подтвердил нарушений правил.")
            await update_ai_status(f"""❌ ИИ: Бот не ответил → отклонён (#{report_id})""")
            return
        ...
        await update_ai_status(f"""🤖 ИИ проанализировал (#{report_id})
📌 Рекомендация: {decision['reason_type'].upper()} ({decision['confidence']:.2f})
🔧 Метод: InputReportReason{decision['reason_type'].capitalize()}()
📋 Передан модераторам""")
    except Exception as e:
        print(f"AI fatal error: {e}")
        update_report_status(report_id, "pending")
        await notify_all_mods_new_report(report_id)
        await update_ai_status(f"""❌ ИИ ошибка (#{report_id}) — передан в очередь""")

# ===================== ФОНОВАЯ ОЧЕРЕДЬ ИИ =====================
async def ai_queue_processor():
    print("✅ ИИ очередь запущена — только анализирует ботов и выбирает причину")
    while True:
...
def get_statistics():
    try:
        ...
        return {
            ...
            "auto_moderation": "ВКЛЮЧЕНА" if auto_enabled else "ВЫКЛЮЧЕНА"
        }
    except:
        return {"total_users": 0, "total_reports": 0, "pending_reports": 0, "accepted_bots": 0, "rejected_bots": 0, "accepted_channels": 0, "rejected_channels": 0, "auto_moderation": "ОШИБКА"}
...
✅ Ботов удалено: <b>{s['accepted_bots']}</b>
❌ Ботов отклонено: <b>{s['rejected_bots']}</b>
✅ Каналов/чатов принято: <b>{s['accepted_channels']}</b>
❌ Каналов/чатов отклонено: <b>{s['rejected_channels']}</b>
👥 Всего пользователей: <b>{s['total_users']}</b>
📋 Всего репортов: <b>{s['total_reports']}</b>
⏳ В очереди: <b>{s['pending_reports']}</b>
🤖 Авто-модерация ИИ (боты): <b>{s['auto_moderation']}</b>"""
...
async def export_data(query: CallbackQuery):
    ...
    title = "БОТЫ" if is_bot else "КАНАЛЫ И ЧАТЫ"
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    if is_bot:
    ...
    for row in rows:
        ...
        status_text = "ПРИНЯТО" if status == "measures_taken" else "ОТКЛОНЕНО" if status in ("rejected", "banned") else status
        content += f"№{r_id} | {date}\nОт: @{username or 'no_username'}\nСсылка: {link}\nПричина: {reason}\nСтатус: {status_text}\n{'─'*50}\n\n"
    document = BufferedInputFile(content.encode("utf-8"), filename=f"{export_type}_report.txt")
    await mod_bot.send_document(chat_id=query.message.chat.id, document=document, caption=f"📤 Экспорт: {title}")
...
async def cmd_start_ai_queue(message: Message):
    ...
    await message.answer("✅ Авто-модерация ИИ <b>включена</b> (только боты).", parse_mode="HTML")
...
async def cmd_stop_ai_queue(message: Message):
    ...
    await message.answer("❌ Авто-модерация ИИ <b>выключена</b>.", parse_mode="HTML")
...
async def cmd_start_moderation_content(message: Message):
    if not is_moderator(message.from_user.id):
        return
    kb_list = [
        [InlineKeyboardButton(text="📊 Статистика", callback_data="mod_menu:statistic")],
        [InlineKeyboardButton(text="📤 Экспорт ботов", callback_data="export:bots")],
        [InlineKeyboardButton(text="📤 Экспорт каналов", callback_data="export:channels")],
        [InlineKeyboardButton(text="📢 Рассылка пользователям", callback_data="mod_menu:broadcast_users")],
        [InlineKeyboardButton(text="📢 Рассылка модераторам", callback_data="mod_menu:broadcast_mods")],
        [InlineKeyboardButton(text="🤖 Включить ИИ", callback_data="mod_menu:start_ai")],
        [InlineKeyboardButton(text="🤖 Выключить ИИ", callback_data="mod_menu:stop_ai")],
        [InlineKeyboardButton(text="📋 Взять следующий репорт", callback_data="mod_menu:next")]
    ]
    if message.from_user.id == MOD_ID:
        kb_list.append([InlineKeyboardButton(text="👮 Добавить модератора", callback_data="mod_menu:addmod")])
        kb_list.append([InlineKeyboardButton(text="👮 Удалить модератора", callback_data="mod_menu:removemod")])
...
async def mod_menu_callback(query: CallbackQuery, state: FSMContext):
    ...
    if action == "start_ai":
        set_auto_moderation(True)
        await query.message.edit_text("✅ Авто-модерация ИИ включена (только боты).")
    elif action == "stop_ai":
        set_auto_moderation(False)
        await query.message.edit_text("❌ Авто-модерация ИИ выключена.")
    elif action == "statistic":
        await query.message.answer("📊 Отправьте команду /statistic")
    elif action == "addmod":
        if query.from_user.id != MOD_ID:
            await query.answer("Нет прав!")
            return
        await query.message.edit_text("Отправьте /addmod <user_id>")
    elif action == "removemod":
        if query.from_user.id != MOD_ID:
            await query.answer("Нет прав!")
            return
        await query.message.edit_text("Отправьте /removemod <user_id>")
    elif action == "broadcast_users":
        if query.from_user.id != MOD_ID:
            await query.answer("Только главный!")
            return
        await query.message.edit_text("📢 Введите текст для рассылки пользователям:\n/cancel — отмена")
        await state.set_state(BroadcastStates.waiting_text_users)
    elif action == "broadcast_mods":
        if query.from_user.id != MOD_ID:
            await query.answer("Только главный!")
            return
        await query.message.edit_text("📢 Введите текст для рассылки модераторам:\n/cancel — отмена")
        ...
        if query.from_user.id != MOD_ID:
            await query.answer("Нет прав!")
            return
        await query.message.edit_text("🔑 Введите ключи через запятую:\n/cancel — отмена", parse_mode="HTML")
...
async def process_openrouter_apis(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"✅ Сохранено {len(keys)} ключей OpenRouter.")
        await state.clear()
    except:
...
async def cmd_next(message: Message):
    ...
    if next_report:
        ...
        await message.answer("✅ Следующий репорт отправлен вам!")
    else:
        await message.answer("✅ Очередь пуста. Новые репорты будут уведомлены всем модераторам.")
...
async def cmd_addmod(message: Message):
    if message.from_user.id != MOD_ID:
        await message.answer("❌ Только главный модератор.")
        return
    try:
        new_id = int(message.text.split()[1])
        if add_moderator(MOD_ID, new_id):
            await message.answer(f"✅ Модератор {new_id} добавлен.")
        else:
...
async def cmd_removemod(message: Message):
    if message.from_user.id != MOD_ID:
        await message.answer("❌ Только главный.")
        return
    try:
        uid = int(message.text.split()[1])
        if remove_moderator(uid):
            await message.answer(f"✅ Модератор {uid} снят.")
        else:
            await message.answer("⚠️ Не удалось.")
    except:
...
async def cmd_help(message: Message):
    if not is_moderator(message.from_user.id):
        return
    text = """<b>🛠 Полный список команд</b>
• /start — панель модератора
• /statistic — статистика
• /next — взять следующий репорт
• /mods — список модераторов
• /addmod /removemod (только главный)
• /startautomoderationocher — включить ИИ
• /stopautomoderationocher — выключить ИИ
Репорты раскидываются ВСЕМ модераторам. Таймаут 5 минут → эскалация к главному."""
    await message.answer(text, parse_mode="HTML")
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if is_user_banned(user_id):
        await message.answer("Вы заблокированы за нарушение законов Исландии")
        return
    if time.time() - (get_last_report_time(user_id) or 0) < COOLDOWN_SECONDS:
        await message.answer("Пожалуйста, подождите 1 минуту.")
        return
    await message.answer("Здравствуйте! Пожалуйста, опишите вашу проблему, и наши модераторы рассмотрят её:")
...
async def process_description(message: Message, state: FSMContext):
    ...
    await message.answer("Введите причину вашей жалобы:")
    await state.set_state(Form.reason)

@user_dp.message(Form.reason)
async def process_reason(message: Message, state: FSMContext):
    await state.update_data(reason=message.text)
    await message.answer("Введите ссылку на контент, который нарушает правила:")
...
async def process_link(message: Message, state: FSMContext):
    ...
    summary = f"""Проверьте правильность информации:
Описание: {data.get('description', '')}
Причина: {data.get('reason', '')}
Ссылка: {data.get('link', '')}
Всё верно?"""
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Да", callback_data="user_confirm:yes")],
        [InlineKeyboardButton(text="Нет", callback_data="user_confirm:no")]
    ])
...
async def process_confirm(query: CallbackQuery, state: FSMContext):
    ...
    if action == "no":
        await query.message.edit_text("Вы всегда можете использовать команду /start.")
        await state.clear()
        return
    data = await state.get_data()
    ...
    if report_id == -1:
        await user_bot.send_message(user_id, "Не удалось отправить жалобу.")
        await state.clear()
        return
    await user_bot.send_message(user_id, "Ваша жалоба отправлена на модерацию.")
    report_type = determine_report_type(link)
    if report_type == "bot" and is_auto_moderation_enabled():
        print(f"[AI Queue] Бот #{report_id} добавлен в очередь ИИ")
    else:
        await notify_all_mods_new_report(report_id)
    await state.clear()
...
async def mod_action(query: CallbackQuery):
    ...
    if report.get("assigned_to") and report["assigned_to"] != query.from_user.id:
        await query.answer("Этот репорт у другого модератора!")
        return
    ...
    if action == "reject":
        ...
        await user_bot.send_message(user_id, "Ваша жалоба принята на рассмотрение.")
        ...
        await user_bot.send_message(user_id, "По вашей жалобе приняты меры.\n\nСпасибо, что помогаете нам поддерживать безопасность платформы!")
        conn = sqlite3.connect(DB_NAME)
        cur = conn.cursor()
        cur.execute("UPDATE reports SET handled_by = ? WHERE id = ?", (current_mod, report_id))
        conn.commit()
        conn.close()
        await query.answer("Меры приняты")
        await query.message.delete()
        next_r = get_next_unassigned_pending_report()
        if next_r:
            await assign_and_notify_mod(next_r[0], current_mod)
        ...
        await user_bot.send_message(user_id, "Вы заблокированы за нарушение законов Исландии")
...
async def main():
    ...
    print("✅ ИИ теперь ТОЛЬКО собирает материал и выбирает причину (personal_details / spam / other)")
    print("✅ Решение принимает МОДЕРАТОР")
    print("✅ 3 разных текста жалобы на каждую категорию")
    print("✅ Показывается метод: InputReportReasonPersonalDetails() / Spam() / Other()")
    print("Bot launched.")
